	...
	for subset in P: 
		...
		if is_vertex_cover(G,subset):
			...
			if len(subset) < len(best_cover):
				best_cover = subset
	#print(count)
	return best_cover # This is the maximal vertex cover provided as an incorrect example. You want the true minimum cover instead!
...
def is_vertex_cover(G,subset):
	...
	for node in G.V:
		if node not in subset:
			if not set(G.E[node]).issubset(subset):
				return False
	return True
